...
from eval import run_eval_mape, run_eval_rmse
...
def main(opt):
    ...
    for ep in range(1, opt.pre_epoch + 1):
        ...
        test_score = run_eval_rmse(model, test_loader, opt)
        test_score_mape = run_eval_mape(model, test_loader, opt)
        print("   Training loss: {:.4f}\n   Test loss: {:.4f}".format(train_loss, test_loss))
        print("Test score RMSE: {:.3f}".format(test_score))
        print("Test score MAPE: {:.3f}".format(test_score_mape))
